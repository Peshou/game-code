...
bool GameCodeApp::InitInstance(HINSTANCE hInstance, LPWSTR lpCmdLine, HWND hWnd, int screenWidth, int screenHeight)
{
	...
	while (!resourceCheck)
	{
		const DWORDLONG physicalRAM = 1024 * MEGABYTE;
		const DWORDLONG virtualRAM = 1024 * MEGABYTE;
		const DWORDLONG diskSpace = 30 * MEGABYTE;
		if (!CheckStorage(diskSpace))
			return false;

		const DWORD minCpuSpeed = 1600;			// 1.3Ghz
	...
	m_ResCache = GCC_NEW ResCache(100, zipFile);
