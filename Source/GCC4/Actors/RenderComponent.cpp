...
const char* SpaceshipRenderComponent::g_Name = "SpaceshipRenderComponent";
const char* EnemySpaceshipRenderComponent::g_Name = "EnemySpaceshipRenderComponent";
const char* AsteroidRenderComponent::g_Name = "AsteroidRenderComponent";
...
const char* LaserRenderComponent::g_Name = "LaserRenderComponent";
...
shared_ptr<SceneNode> LaserRenderComponent::VCreateSceneNode(void)
{
	// get the transform component
	shared_ptr<TransformComponent> pTransformComponent = MakeStrongPtr(m_pOwner->GetComponent<TransformComponent>(TransformComponent::g_Name));
	if (pTransformComponent)
	{
		...
		case GameCodeApp::Renderer_D3D9:
			return shared_ptr<SceneNode>(GCC_NEW D3DLaserMeshNode9(m_pOwner->GetId(), weakThis, "Effects\\GameCode4.fx", RenderPass_Actor, &pTransformComponent->GetTransform()));
		...
		case GameCodeApp::Renderer_D3D11:
		{
			...
			shared_ptr<SceneNode> teapot(GCC_NEW D3DLaserMeshNode11(INVALID_ACTOR_ID, weakThis, RenderPass_Actor, &rot90));
			parent->VAddChild(teapot);
			return parent;
		}

		default:
			GCC_ERROR("Unknown Renderer Implementation in LaserRenderComponent");
		}
	}
...
void LaserRenderComponent::VCreateInheritedXmlElements(TiXmlElement *)
{
}
//---------------------------------------------------------------------------------------------------------------------
// SpaceshipRenderComponent
//---------------------------------------------------------------------------------------------------------------------
shared_ptr<SceneNode> SpaceshipRenderComponent::VCreateSceneNode(void)
{
    // get the transform component
    shared_ptr<TransformComponent> pTransformComponent = MakeStrongPtr(m_pOwner->GetComponent<TransformComponent>(TransformComponent::g_Name));
    if (pTransformComponent)
    {
		...
		{
			case GameCodeApp::Renderer_D3D9: 
				return shared_ptr<SceneNode>(GCC_NEW D3DSpaceshipMeshNode9(m_pOwner->GetId(), weakThis, "Effects\\GameCode4.fx", RenderPass_Actor, &pTransformComponent->GetTransform()));
			...
			case GameCodeApp::Renderer_D3D11: 
			{
				...
				rot90.BuildRotationY(-GCC_PI/2.0f);
				shared_ptr<SceneNode> parent(GCC_NEW SceneNode(m_pOwner->GetId(), weakThis, RenderPass_Actor, &pTransformComponent->GetTransform()));
				shared_ptr<SceneNode> teapot(GCC_NEW D3DSpaceshipMeshNode11(INVALID_ACTOR_ID, weakThis, RenderPass_Actor, &rot90));
				parent->VAddChild(teapot);
				return parent;
			}
								 
			default:
				GCC_ERROR("Unknown Renderer Implementation in SpaceshipRenderComponent");
		}
    }
...
void SpaceshipRenderComponent::VCreateInheritedXmlElements(TiXmlElement *)
{
}
...
// SpaceshipRenderComponent
//---------------------------------------------------------------------------------------------------------------------
shared_ptr<SceneNode> AsteroidRenderComponent::VCreateSceneNode(void)
{
	// get the transform component
	shared_ptr<TransformComponent> pTransformComponent = MakeStrongPtr(m_pOwner->GetComponent<TransformComponent>(TransformComponent::g_Name));
	if (pTransformComponent)
	{
		...
		{
		case GameCodeApp::Renderer_D3D9:
			return shared_ptr<SceneNode>(GCC_NEW D3DAsteroidMeshNode9(m_pOwner->GetId(), weakThis, "Effects\\GameCode4.fx", RenderPass_Actor, &pTransformComponent->GetTransform()));
		...
		case GameCodeApp::Renderer_D3D11:
		{
			...
			rot90.BuildRotationY(-GCC_PI / 2.0f);
			shared_ptr<SceneNode> parent(GCC_NEW SceneNode(m_pOwner->GetId(), weakThis, RenderPass_Actor, &pTransformComponent->GetTransform()));
			shared_ptr<SceneNode> teapot(GCC_NEW D3DAsteroidMeshNode11(INVALID_ACTOR_ID, weakThis, RenderPass_Actor, &rot90));
			parent->VAddChild(teapot);
			return parent;
		}
...
void AsteroidRenderComponent::VCreateInheritedXmlElements(TiXmlElement *)
{
}
...
// EnemyShipComponent
//---------------------------------------------------------------------------------------------------------------------
shared_ptr<SceneNode> EnemySpaceshipRenderComponent::VCreateSceneNode(void)
{
	// get the transform component
	shared_ptr<TransformComponent> pTransformComponent = MakeStrongPtr(m_pOwner->GetComponent<TransformComponent>(TransformComponent::g_Name));
	if (pTransformComponent)
	{
		...
		{
		case GameCodeApp::Renderer_D3D9:
			return shared_ptr<SceneNode>(GCC_NEW D3DEnemySpaceshipMeshNode9(m_pOwner->GetId(), weakThis, "Effects\\GameCode4.fx", RenderPass_Actor, &pTransformComponent->GetTransform()));
		...
		case GameCodeApp::Renderer_D3D11:
		{
			...
			rot90.BuildRotationY(-GCC_PI / 2.0f);
			shared_ptr<SceneNode> parent(GCC_NEW SceneNode(m_pOwner->GetId(), weakThis, RenderPass_Actor, &pTransformComponent->GetTransform()));
			shared_ptr<SceneNode> teapot(GCC_NEW D3DEnemySpaceshipMeshNode11(INVALID_ACTOR_ID, weakThis, RenderPass_Actor, &rot90));
			parent->VAddChild(teapot);
			return parent;
		}

		default:
			GCC_ERROR("Unknown Renderer Implementation in EnemySpaceshipRenderComponent");
		}
	}
...
void EnemySpaceshipRenderComponent::VCreateInheritedXmlElements(TiXmlElement *)
{
}
