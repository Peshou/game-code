...
MessageBox::MessageBox(std::wstring msg, std::wstring title, int buttonFlags)
{
	...
	m_Width = std::max((int)(msgWidth*1.5) + 2 * border, btnWidth + 2 * border);
	...
	D3DCOLOR indigo = D3DXCOLOR(0.24f, 0.31f, 0.71f, 1.0f);
	m_UI.SetBackgroundColors(indigo);

	int iY = border;
	int iX = (m_Width - msgWidth) / 2;

	m_UI.AddStatic(0, msg.c_str(), 0, iY, msgWidth, msgHeight);
...
int MessageBox::Ask(MessageBox_Questions question)
{
	...
	case MESSAGE_OBJECTIVES:
	{
		msg = g_pApp->GetString(_T("IDS_MESSAGES_OBJECTIVES"));
		title = g_pApp->GetString(_T("IDS_OBJECTIVES"));
		buttonFlags = MB_OK;
		defaultAnswer = IDOK;
		break;
	}
