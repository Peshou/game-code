...
#include "../GCC4/ResourceCache/XmlResource.h"
...
extern SpaceWarsApp g_SpaceWarsApp;
...
MainMenuUI::MainMenuUI()
{
	...
	//GCC_ASSERT(mainMenuResource);

	//// load all materials
	//TiXmlElement* pParentNode = mainMenuResource->FirstChildElement("PhysicsMaterials");
	//GCC_ASSERT(pParentNode);

	//m_SampleUI.SetTexture
	//	
	...
	m_SampleUI.GetSlider( CID_NUM_AI_SLIDER )->SetRange(3, g_pApp->m_Options.m_maxAIs);
...
{
	HRESULT hr;
	DXUT_BeginPerfEvent( DXUT_PERFEVENTCOLOR, L"SpaceWarsHUD" ); // These events are to help PIX identify what the code is doing
...
{
	//D3DCOLOR(0.07f, 0.29f, 0.12f)
	HRESULT hr;
	DXUT_BeginPerfEvent(DXUT_PERFEVENTCOLOR3, L"StandardUI" ); // These events are to help PIX identify what the code is doing
...
SpaceWarsHumanView::SpaceWarsHumanView(shared_ptr<IRenderer> renderer) : HumanView(renderer)
{ 
	m_bShowUI = true; 
...
SpaceWarsHumanView::~SpaceWarsHumanView()
{
    RemoveAllDelegates();
}

//
// SpaceWarsHumanView::VOnMsgProc				- Chapter 19, page 727
//
LRESULT CALLBACK SpaceWarsHumanView::VOnMsgProc( AppMsg msg )
{
	if (HumanView::VOnMsgProc(msg))
		return 1;
	...
	if (msg.m_uMsg==WM_KEYDOWN)
	{
		...
		{
			SpaceWarsLogic *twg = static_cast<SpaceWarsLogic *>(g_pApp->m_pGame);
			twg->ToggleRenderDiagnostics();

		}
		else if (msg.m_wParam==VK_F9)
		{
			m_KeyboardHandler = m_pSpaceshipController;
			m_PointerHandler = m_pSpaceshipController;
			m_pCamera->SetTarget(m_pSpaceship);
			m_pSpaceship->SetAlpha(1.0f);
			ReleaseCapture();
			return 1;
		}
...
//
// SpaceWarsHumanView::VRenderText				- Chapter 19, page 727
//
void SpaceWarsHumanView::VRenderText()
{
...
//
void SpaceWarsHumanView::VOnUpdate(unsigned long deltaMs)
{
	HumanView::VOnUpdate( deltaMs );
	...
	if (m_pSpaceshipController)
	{
		m_pSpaceshipController->OnUpdate(deltaMs);
	}
...
//
void SpaceWarsHumanView::VOnAttach(GameViewId vid, ActorId aid)
{
	HumanView::VOnAttach(vid, aid);
}

bool SpaceWarsHumanView::VLoadGameDelegate(TiXmlElement* pLevelData)
{
...
void SpaceWarsHumanView::VSetControlledActor(ActorId actorId)
{ 
	m_pSpaceship = static_pointer_cast<SceneNode>(m_pScene->FindActor(actorId));
    if (!m_pSpaceship)
    {
        GCC_ERROR("Invalid spaceship");
        return;
    }

	HumanView::VSetControlledActor(actorId);

    m_pSpaceshipController.reset(GCC_NEW SpaceshipController(m_pSpaceship));
    m_KeyboardHandler = m_pSpaceshipController;
    m_PointerHandler = m_pSpaceshipController;
    m_pCamera->SetTarget(m_pSpaceship);
	m_pSpaceship->SetAlpha(1.0f);
...
void SpaceWarsHumanView::GameplayUiUpdateDelegate(IEventDataPtr pEventData)
{
...
void SpaceWarsHumanView::SetControlledActorDelegate(IEventDataPtr pEventData)
{
...
//
AISpaceshipView::AISpaceshipView(shared_ptr<PathingGraph> pPathingGraph) : IGameView(), m_pPathingGraph(pPathingGraph)
{
    //
}
...
// AITeapotView::~AITeapotView					- Chapter 19, page 737
//
AISpaceshipView::~AISpaceshipView(void)
{
    GCC_LOG("AI", "Destroying AISpaceshipView");
}
