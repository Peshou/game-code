...
class SpaceWarsHumanView : public HumanView
{
protected:
	bool  m_bShowUI;					// If true, it renders the UI control text
    std::wstring m_gameplayText;

	shared_ptr<SpaceshipController> m_pSpaceshipController;
	shared_ptr<MovementController> m_pFreeCameraController;
	shared_ptr<SceneNode> m_pSpaceship;
	shared_ptr<StandardHUD> m_StandardHUD; 

public:
	SpaceWarsHumanView(shared_ptr<IRenderer> renderer);
	virtual ~SpaceWarsHumanView();
...
class AISpaceshipView : public IGameView 
{
	friend class AISpaceshipViewListener;
...
public:
	AISpaceshipView(shared_ptr<PathingGraph> pPathingGraph);
	virtual ~AISpaceshipView();
