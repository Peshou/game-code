...
function ActorManager:OnFireWeapon(eventData)
	...
	local ypr = Vec3:Create({x=0, y=0, z=90});
	-- local laser_fire = CreateActor("actors\\sphere.xml", pos, ypr);
	local laser_fire = CreateActor("actors\\laser_fire.xml", pos, ypr);
	if (laser_fire ~= -1) then
		dir:Normalize();
		ApplyForce(dir, 300, laser_fire);
	end
end
...
function ActorManager:_DestroyOnAsteroidCollision(spaceship)
	if (spaceship == self._player) then
		QueueEvent(EventType.EvtData_Destroy_Player, spaceship:GetActorId());
	else
		self:RemoveEnemy(spaceship);
		QueueEvent(EventType.EvtData_Request_Destroy_Actor, spaceship:GetActorId());
	end
...
function ActorManager:OnPhysicsCollision(eventData)
	...
	if(spaceship ~= nil) then
		self:_DestroyOnAsteroidCollision(spaceship);
		return;
	end
	...
	-- play the hit sound
	QueueEvent(EventType.EvtData_PlaySound, "audio\\decompression.wav");
end
...
function ActorManager:UpdateUi()
	...
	if (self._enemies ~= nil) then
		for id, spaceship in pairs(self._enemies) do
			uiText = uiText .. "Spaceship with id ->" .. id .. " has " .. spaceship.hitPoints .. " HP\n";
		end
	end
...
function ActorManager:_DamageTeapot(spaceship)
	-- [rez] DEBUG: player is immune for now....
	-- subtract a hitpoint
	spaceship.hitPoints = spaceship.hitPoints - 1;

	-- if the spaceship is dead, destroy it, otherwise update the UI
	if (spaceship.hitPoints <= 0) then
		if (spaceship == self._player) then
